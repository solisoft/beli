  ...
  for (var i = 0; i < imgs.length; ++i) {
    ...
    if(src && src.indexOf("data:image") == -1) {
      if(item.width >= 214 && item.height >= 100 && y < max) {
        html += "<div style='float:left; margin: 10px;width:150px; height: 90px; overflow: hidden;'><a href='http://dev.24s46.com/import/"+encodeURIComponent(src)+"/"+encodeURIComponent(document.location.href)+"/"+encodeURIComponent(text)+"' alt='"+text.replace(/'/g, "\\'")+"' title='"+text.replace(/'/g, "\\'")+"' target='beli'><img src='" + src + "' width='150' style='box-shadow: 0 0 10px #000;border-radius: 4px;'></a></div>";
        y++;
      }
    }
